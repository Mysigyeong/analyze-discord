...
static u8 save_if_interesting(char** argv, void* mem, u32 len, u8 fault) {
  ...
  FILE* cmd_ptr = popen("ps -aux | awk \'/Discord --type=renderer/ && $11 !~ /awk/ { print $2 }\'", "r");
  if (cmd_ptr == NULL) {
	puts("popen failed");
	exit(1);
  }
  char cmd_temp[100] = { 0 };
  int temp_pid = 0;
  fgets(cmd_temp, 100, cmd_ptr);

  pclose(cmd_ptr);

  temp_pid = atoi(cmd_temp);

  if (temp_pid == 0) {
    fault = FAULT_CRASH;
  }
  else if (temp_pid != discord_pid) {
    fault = FAULT_CRASH;
	discord_pid = temp_pid;
  }
...
int main(int argc, char** argv) {
  ...
  discord_pid = atoi(cmd_temp);

  if (discord_pid == 0) {
	puts("discord_pid is 0");
	exit(1);
  }
